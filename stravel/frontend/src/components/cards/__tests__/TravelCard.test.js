...
describe('TravelCard — nascent state', () => {
    it('renders article role', () => {
        render(_jsx(TravelCard, { ...baseProps }));
        expect(screen.getByRole('article')).toBeInTheDocument();
    });
    it('has correct aria-label with card type and percentage', () => {
        render(_jsx(TravelCard, { ...baseProps, completenessScore: 0.1 }));
        expect(screen.getByRole('article')).toHaveAttribute('aria-label', 'flight card, 10% complete');
    });
    it('renders sr-only live region', () => {
        const { container } = render(_jsx(TravelCard, { ...baseProps }));
    ...
    it('applies will-change-transform class when shimmerEnabled=true', () => {
        const { container } = render(_jsx(TravelCard, { ...baseProps, shimmerEnabled: true }));
    ...
    it('does not apply will-change-transform when shimmerEnabled=false', () => {
        const { container } = render(_jsx(TravelCard, { ...baseProps, shimmerEnabled: false }));
...
describe('TravelCard — forming state', () => {
    ...
    it('shows percentage in footer', () => {
        const { container } = render(_jsx(TravelCard, { ...formingProps }));
    ...
    it('aria-label reflects forming score', () => {
        render(_jsx(TravelCard, { ...formingProps }));
        expect(screen.getByRole('article')).toHaveAttribute('aria-label', 'flight card, 50% complete');
    });
});
describe('TravelCard — settled state', () => {
    ...
    it('shows edit button when onEdit provided', () => {
        const onEdit = vi.fn();
        render(_jsx(TravelCard, { ...settledProps, onEdit: onEdit }));
    ...
    it('does not show Book button in browsing deckState', () => {
        render(_jsx(TravelCard, { ...settledProps, onBook: vi.fn(), deckState: "browsing" }));
        expect(screen.queryByRole('button', { name: /book/i })).not.toBeInTheDocument();
    });
    it('shows Book button in committing deckState', () => {
        render(_jsx(TravelCard, { ...settledProps, onBook: vi.fn(), deckState: "committing" }));
...
describe('TravelCard — error state (stall timeout)', () => {
    ...
    it('shows error message after 90s stall', () => {
        render(_jsx(TravelCard, { ...baseProps, completenessScore: 0.1, isFinal: false }));
    ...
    it('calls onRetry when Try again is clicked', () => {
        const onRetry = vi.fn();
        render(_jsx(TravelCard, { ...baseProps, completenessScore: 0.1, isFinal: false, onRetry: onRetry }));
        act(() => { vi.advanceTimersByTime(90_001); });
        fireEvent.click(screen.getByRole('button', { name: /try again/i }));
        expect(onRetry).toHaveBeenCalledOnce();
    });
    it('does not stall when not nascent (forming)', () => {
        render(_jsx(TravelCard, { ...baseProps, completenessScore: 0.5, isFinal: false }));
        act(() => { vi.advanceTimersByTime(90_001); });
        expect(screen.queryByText('Taking longer than expected')).not.toBeInTheDocument();
    });
});
describe('TravelCard — card types', () => {
    it('renders hotel card type', () => {
        render(_jsx(TravelCard, { ...baseProps, cardType: "hotel", completenessScore: 0.5 }));
        expect(screen.getByRole('article')).toHaveAttribute('aria-label', expect.stringContaining('hotel card'));
        expect(screen.getByText('hotel')).toBeInTheDocument();
    });
    it('renders activities card type', () => {
        render(_jsx(TravelCard, { ...baseProps, cardType: "activities", completenessScore: 0.5 }));
        expect(screen.getByRole('article')).toHaveAttribute('aria-label', expect.stringContaining('activities card'));
    });
    it('renders visa card type', () => {
        render(_jsx(TravelCard, { ...baseProps, cardType: "visa", completenessScore: 0.5 }));
        expect(screen.getByRole('article')).toHaveAttribute('aria-label', expect.stringContaining('visa card'));
    });
    it('renders flight structural fields in forming state', () => {
        render(_jsx(TravelCard, { ...baseProps, cardType: "flight", completenessScore: 0.5, delta: { origin: 'HAN', destination: 'SGN' } }));
