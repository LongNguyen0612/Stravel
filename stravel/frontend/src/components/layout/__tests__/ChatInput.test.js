...
describe('ChatInput', () => {
    it('renders an input and a send button', () => {
        render(_jsx(ChatInput, { onSubmit: vi.fn() }));
        expect(screen.getByTestId('chat-input')).toBeInTheDocument();
        expect(screen.getByTestId('chat-send')).toBeInTheDocument();
    });
    it('calls onSubmit with the trimmed input value', () => {
        const onSubmit = vi.fn();
        render(_jsx(ChatInput, { onSubmit: onSubmit }));
    ...
    it('disables input and button when disabled=true', () => {
        render(_jsx(ChatInput, { onSubmit: vi.fn(), disabled: true }));
    ...
    it('uses custom placeholder', () => {
        render(_jsx(ChatInput, { onSubmit: vi.fn(), placeholder: "Where to?" }));
    ...
    it('forwards ref to the outer container div', () => {
        const ref = createRef();
        render(_jsx(ChatInput, { ref: ref, onSubmit: vi.fn() }));
